...
if( count($les_arbres ) > 0 ) {
	...
	if( ($action=='A0' || $action=='A1' || $action=='A2')  && count($les_modalites) > 0 ) {
		...
		//si c'est une modalité multiple(donc avec critères)
		//et si 'id_choix' est en parametre
...
function afficher_liste_modalites($les_modalites, $id_modalite_classe_select) {
	...
	global $param_crit;
	echo '
<select name="id_modalite_classe" 
	onChange="this.form.elements[\'action\'].value=\'A3\'; this.form.submit();">';
	...
	foreach( $les_modalites  as $modalite ){
		...
		if(strtolower(get_class($modalite))=='modalite_va_unique' && $modalite->type_reponse=='texte') continue; //ignorer les modalités Ã  réponse texte 
...
function afficher_liste_choix($les_choix, $id_choix_select) {
	...
	echo '
<select name="id_choix" 
	onChange="this.form.elements[\'action\'].value=\'A4\'; this.form.submit();">';

	echo'
	<option value="-1" >Choix du crit&egrave;re</option>';
	if(isset($_REQUEST['id_modalite_classe']) && $_REQUEST['id_modalite_classe'] != -1){
		//Boucle de remplissage de la liste déroulante
...
/***
 * Cette fonction affiche la barre de progression qui correspond a la valeur $val
 * $val <=0	 affiche 0%
		...
		if($param_crit->mode_affichage !="aucun"){
			...
			if($param_crit->mode_affichage =="tout" || $param_crit->mode_affichage =="graphique"){
				...
				if($param_crit->mode_graphique == "bps"){
					if($width<50){
						echo "<div class=\"fondbarre\">	<div class=\"barrerouge\" style=\"width: ".$width."px;\">";
						...
						echo "<div class=\"fondbarre\">	<div class=\"barrebleu\" style=\"width: ".$width."px;\">";
						//Verif si il faut afficher le texte ou non
						if($param_crit->mode_textuel == "calcule"){
							echo $width."%	</div></div>";
						} else{
				...
				//bpp: Barre de Progression a Paliers
				if($param_crit->mode_graphique == "bpp"){
					if($width<50){
						echo "<div class=\"fondbarre\">	<div class=\"barrepallierrouge\" style=\"width: ".$width."px;\">";
						...
						echo "<div class=\"fondbarre\">	<div class=\"barrepallierbleu\" style=\"width: ".$width."px;\">";
						...
						case 2:
							if($width > $tabParamSmil[0]){
								if($width > $tabParamSmil[1]){
									echo "<div class='smiley_5'></div>";
								}else{
									echo "<div class='smiley_3'></div>";
								}
							}else{
								echo "<div class='smiley_1'></div>";
							}
							break;
						case 3:
							if($width > $tabParamSmil[0]){
								if($width > $tabParamSmil[1]){
									if($width > $tabParamSmil[2]){
										...
										echo "<div class='smiley_4'></div>";
									}
								}else{
									echo "<div class='smiley_2'></div>";
								}
							}else{
								echo "<div class='smiley_1'></div>";
							}
...
<ul>
	<li class="arbre">';progressbar(round(eval_noeud(0), 2));
		...
		echo"<h2>Vous n'avez pas de suivi guid&eacute; pour ";
