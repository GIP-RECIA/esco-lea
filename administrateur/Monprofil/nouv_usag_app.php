...
<div id="top_l"></div>
<div id="top_m">
	<h1><span class="orange">A</span>jouter <?php echo $config_term->terminologie_app; ?></h1>
</div>
<div id="top_r"></div>
<div id="m_contenu">
<?php echo"
	<form name='theForm' action='form_nouv_usag_v.php?id_usager=".$id_app."&profil=app&action=".$action."' method='post' onsubmit='return controleSaisie(this)' enctype='multipart/form-data'>" ?>
		<table width="69%"  border="0" cellpadding="0" cellspacing="0">
  			<tr>
    			<th height="37" colspan="2">Informations <?php echo $config_term->terminologie_app; ?></th>
  			</tr>
			<tr>
			  	<td height="29"><?php echo $config_term->terminologie_classe; ?></td>
			  	<td>
			  	<?php 			  			  
					echo" 
					<select name='id_cla'>";
					echo" 
					 ...
					 if (count($les_classes) > 0){
					 	foreach ($les_classes as $classe){	  			  
							if ($classe->id_cla == $apprenti->id_cla) $selected="selected=\"selected\"";
							else $selected="";
							echo"
			...
			<tr>
			  	...
			  	<td><input name="tel_fixe" type="text" value='<?php echo "$apprenti->tel_fixe"?>'></td>
			</tr>
			<tr>
			  	<td height="18">T&eacute;l&eacute;phone portable</td>
			  	<td><input name="tel_mobile" type="text" value='<?php echo "$apprenti->tel_mobile"?>'></td>
			</tr>
			<tr>
			  	<td height="22">E-mail </td>
			  	<td><input name="email" type="text" value='<?php echo "$apprenti->email"?>' size="40"></td>
			</tr>
			<tr>
			  	<td height="21">Site web(url)</td>
			  	<td><input name="url_site" type="text"value='<?php echo "$apprenti->url_site"?>' size="40"></td>
			</tr>
			<tr>
			...
			<tr>
			  	...
			  	<td><input name="no_insc" type="text" value='<?php echo "$apprenti->no_insc"?>'></td>
			</tr>
			<tr>
			  	<td height="23" bordercolor="#99CCFF">Num&eacute;ro de s&eacute;curit&eacute; sociale</td>
			  	<td><input name="no_secu" type="text" value='<?php echo "$apprenti->no_secu"?>'></td>
			</tr>
			<tr>
			  	<td height="22">Situation ann&eacute;e p&eacute;c&eacute;dente</td>
			  	<td><input name="dern_classe_freq" type="text" value='<?php echo (to_html($apprenti->dern_classe_freq))?>'></td>
			</tr>
			<tr>
			  	<td height="18">Diplômes obtenus</td>
			  	<td><input name="diplomes_obtenus" type="text" value="<?php echo (to_html($apprenti->diplomes_obtenus)) ?>"></td>
			</tr>
			<tr>
			  	<td height="72">Autre adresse </td>
			  	<td><textarea name="adresse_perso" cols="40" rows="4" ><?php echo (to_html($apprenti->adresse_perso)) ?></textarea></td>
			</tr>
			<tr>
			  	<td height="18">T&eacute;l&eacute;phone personnel</td>
			  	<td><input name="tel_perso" type="text" value='<?php echo "$apprenti->tel_perso"?>' maxlength="30"></td>
			</tr>
			<tr>
			  	<td height="18">E-mail personnel</td>
			  	<td><input name="email_perso" type="text" value='<?php echo "$apprenti->email_perso"?>' size="40"></td>
			</tr>
			<tr>
			...
			<tr >
			  	<th height="22" colspan="2">Suivi <?php echo $config_term->terminologie_app; ?> en <?php echo $config_term->terminologie_entr; ?></th>
			</tr>
			<tr>
					...
					echo"
					 ...
					 if (isset($les_ma)){
					 	foreach ($les_ma as $ma){	  			  
							if ($ma->id_ma== $apprenti->id_ma) $selected="selected=\"selected\"";
							else $selected="";
							echo"
						<option value='$ma->id_ma' $selected > $ma->nom &nbsp $ma->prenom </option>";			  
					 	}
					}				  				
					echo"
					</select>";
			...
			<tr>
			  	<td height="31">Date de debut du contrat</td>
			  	<td>
			...
			<tr >
			  	<th height="23" colspan="2">Suivi <?php echo $config_term->terminologie_app; ?> en <?php echo($config_term->terminologie_cfa); ?></th>
			</tr>
			<tr>
			  	<td height="24">Nom et Pr&eacute;nom <?php echo $config_term->terminologie_ens; ?></td>
				<td>
			    <?php 			  			  
			    	echo" 
					<select name='id_ens'>";
				  	...
				  	if (count($les_enseignants) > 0){
				  		...
				  		foreach ($les_enseignants as $enseignant){
							...
							if ($enseignant->id_ens == $apprenti->id_ens) $selected="selected=\"selected\"";
							else $selected="";
				  			echo" 
						<option value='".$enseignant->id_ens."' ".$selected."> ".$enseignant->nom."&nbsp".$enseignant->prenom." </option>";			  
				  		}
				 	}				  				
			        echo" 
					</select>";
			...
			<tr>
			  	<th height="25" colspan="2"><?php echo $config_term->terminologie_rl; ?></th>
			</tr>
			<tr>
					...
					<select name='id_rl'>";
				  	...
				  	if (isset($les_rl)){
				  		foreach ($les_rl as $rl){	  			  
							if ($rl->id_rl == $apprenti->id_rl) $selected="selected=\"selected\"";
							else $selected="";
					  		echo"
						<option value='".$rl->id_rl."' ".$selected." >".$rl->nom."&nbsp".$rl->prenom." </option>";			  
				  		}
				 	}				  				
			        echo"
					</select>";
