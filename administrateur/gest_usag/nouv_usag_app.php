...
/***********************************************************/
if(isset($_REQUEST['action'])) $action=$_REQUEST['action']; // l'action demandée : mofier ou ajouter un nouvel apprenti
else $action="nouv";
...
$les_rl = $bdd->get_usagers(0,10000, "rl"); 		  //liste de tous les  représentants légaux
...
<div id="top_m">
	<?php
		if ($action == "nouv") {
			echo "<h1><span class='orange'>A</span>jouter ".$config_term->terminologie_app."</h1>";
		} else {
			echo "<h1><span class='orange'>M</span>odifier ".$config_term->terminologie_app."</h1>";			
		}
	?>
</div>
<div id="top_r"></div>
...
<?php echo"
	<form name='theForm' action='form_nouv_usag_v.php?id_usager=".$id_app."&profil=app&action=".$action."' method='post' onsubmit='return controleSaisie(this)' enctype='multipart/form-data'>" ?>
		<table width="69%"  border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
			  	<td height="18">Diplomes obtenus</td>
					...
					echo"
					 ...
					 if (isset($les_ma)){
					 	foreach ($les_ma as $ma){	  			  
							...
							echo"
						<option value='$ma->id_ma' $selected > $ma->nom&nbsp;$ma->prenom </option>";			  
					 	}
					}				  				
					echo"
					</select>";
				  	...
				  	if (isset($les_rl)){
				  		foreach ($les_rl as $rl){	  			  
							if ($rl->id_rl == $apprenti->id_rl) $selected="selected=\"selected\"";
					  		...
					  		echo"
						<option value='".$rl->id_rl."' ".$selected." >".$rl->nom."&nbsp;".$rl->prenom." </option>";			  
				  		}
				 	}				  				
			        echo"
					</select>";
    ...
    <td class="cellule">
        ...
        <input name='droit[]' type='radio' value='app' checked><?php echo $config_term->terminologie_app;?>
