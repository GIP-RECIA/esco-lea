...
if(isset($messages))
{
	...
	// On vide de la corbeille les messages selectionnés
	if (isset($_GET['vider'])) {
		$tabIdMessage = split('\|', $_GET['vider']);
		foreach ($tabIdMessage as $idMessage) {
			// Recherche du message a effacer dans la liste
			foreach ($messages as $i => $value) {
				$message = $messages[$i];
				if ($message->id_msg == $idMessage) {
					$message->delete_message_recu($id_usager);
					unset($messages[$i]);
					break;
				}
			}
	...
	foreach ($messages as $message)
	{
		...
		if ($message->suppression == 'NON') continue;
		else $tab[] = $message;
	}
...
<center style="margin-top: 5px;"><?php  
// Pagination :
echo 'Page : ';
for ($i = 1 ; $i <= $nbPages ; $i++)
{
	echo '<a href="corbeille.php?page=' . $i . '">' . $i . '</a> ';
}
// Fin
	}
}
