window.onload = function ()
{
	...
	{
		var id = suppression[i].getProperty('id');
		if ($('input_' + id + '_lea').getProperty('value') != 'false')
	...
	for (var i = 0 ; i < tab.length ; i++)
	{
		if ($('input_supp_' + tab[i]).getProperty('value') == 'false')
		{
			...
			if ($('input_supp_' + tab[i] + '_lea').getProperty('value') == 'false')
			{
				element.setProperty('class', 'activation');
				element.setProperty('src', 'param/interdiction.jpg');
				element.setProperty('alt', 'interdiction');
			}
			else
			{
				element.setProperty('class', 'activation');
...
function addFunction(element, role)
{
	...
	// Test pour savoir si l'�l�ment est d�j� pr�sent
	...
	// Test pour savoir si la fonction n'est pas celle du r�le (pour ne pas donner le droit de Gestion Web � l'admin, par exemple)
	if (!exist)
	{
