...
$msg_err = $_SESSION['erreur_import'];
...
<script language="JavaScript">
	...
	function controleSaisieImport(theForm)
	{   	    			   
		if(testVide(theForm.nom, "intitul&eacute; de l'arbre")==false ) return false; 
		if(testVide(theForm.fichier, "fichier &agrave; importer")==false ) return false;

		var nomFichier = theForm.fichier.value;

		if ( nomFichier.length<4 || nomFichier.toLowerCase().substr(nomFichier.length-4, 4) != ".xml") {
			alert("Le fichier a importer doit être un fichier XML");
			return false;
		}
...
<div id="contenu">
	...
	<?php
  		if(isset($msg_err)) {
  			echo "<div class='erreur'><img src='".$URL_THEME."images/default_pictoStop_cub.png' />".$msg_err."</div>";
	...
	?>  	
   			...
   			<div id="nouv_ref" style="hidden" >
     			<table width="695" cellspacing="0" >
        			...
        			<tr>
						...
						<td class="cellule">
							<input name="nb_niveaux" type="text" size="4" onBlur="afficher_niveaux(this.value)">niveau(x)
		...
		<form enctype="multipart/form-data" name="formImport" method="post" action="./config_suivi/import_arbre_v.php?type_suivi=<?php echo $_GET['type_suivi']; ?>&suivi=<?php echo $_GET['suivi']; ?>" onSubmit="return controleSaisieImport(this);">
    		<table width="695" cellspacing="0" >
       			<tr>
         			<th height="26" colspan="2">Importer un nouvel arbre</th>
				</tr>
        		<tr>
      				<td width="29%" height="43" class="sous_titre_tableau">
      					Votre arbre s'intitule
        		...
        		<tr align="left">
          			<td height="31">Fichier &agrave; importer</td>
          			<td>
  	        			<input type="hidden" name="MAX_FILE_SIZE" value="50000" />
          				<input name="fichier" type="file" size="50">
          				<input type="hidden" name="idFormation" value="<?php echo $id_for; ?>" >
          				<input type="hidden" name="idConfig" value="<?php echo $config_lea->id_config; ?>" >
          				<input type="hidden" name="typeSuivi" value="<?php echo $_GET['type_suivi']; ?>" >
          				<a href="#" onclick="lightbox('aide_50', '<?php echo $LEA_URL?>')">
          					<img src="<?php echo($LEA_URL.'themes/'.$_SESSION['options_lea']['LEA_THEME'].'/images/ico_aide.gif'); ?>" border="0" />
          				</a>
          			</td>
        		</tr>
        		<tr align="left">
          			<td height="31"></td>
          			<td>
          				<input type="submit" name="Submit" value="Importer cet arbre">
          			</td>
        		</tr>
		...
		<table width="50%" cellspacing="0" >
   			<tr>
   				<th>Liste des arbres d&eacute;j&agrave; cr&eacute;&eacute;s 
					<a href="#" onclick="lightbox('aide_24', '<?php echo $LEA_URL?>')">
						<img src="<?php echo($LEA_URL.'themes/'.$_SESSION['options_lea']['LEA_THEME'].'/images/ico_aide.gif'); ?>" border="0" />
					</a>
				</th>
   			</tr>
...
<?php 
	if (isset($msg_info)) {
		echo '<script>alert("'.$msg_info.'");</script>';
