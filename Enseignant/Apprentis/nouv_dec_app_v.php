...
else  $_SESSION['les_feuilles_modalite_va_unique'] = NULL;
...
else $_SESSION['les_feuilles_modalite_va_multiple'] = NULL;
	...
	if(isset($_FILES['fichier'])) {
			...
			if  (move_uploaded_file($src, $SRC_DOCUMENTS_DECLARES.$dest)) { 						 		 		 		
				if ($dest!=""){                 
					...
					$document->id_usager = $_SESSION['id_ens'] ;				
					$document->insert(); 										
...
// --------- Mise à jour de la confidentialité des fichiers  -------------
...
// --------- enregistrement des feuilles sélectionnées -------------
