...
/*****************************************/
/***
 * RÃ©cupÃ©ration de id_app, et crÃ©ation de l'apprenti
 */
if(isset($_REQUEST['id_app_select'])) $id_app_select = $_REQUEST['id_app_select'];
else { html_refresh($LEA_URL); exit(); }

$apprenti = new Apprenti($id_app_select );
$apprenti->set_detail();
/***
 * VÃ©rification si le maitre apprentissage et le responsable lÃ©gal
 * sont les mÃªm que ceux de l'apprenti
 */
...
$les_id_apprentis = $classe->get_id_apprentis();  // les apprentis de la classe de l'apprenti sï¿½lectionnï¿½.
...
/***
 * RÃ©cupÃ©ration du type de suivi
 * pour rÃ©cupÃ©rer les bons arbres
 */
 ...
 * des noeuds branche de l'arbre reprÃ©sentÃ© par le tableau 
...
/***
 * Cette fonction affiche la liste des modalitÃ©s
 */
function afficher_liste_modalites($les_modalites, $id_modalite_classe_select) {
	...
	foreach( $les_modalites  as $modalite ){
		...
		if(strtolower(get_class($modalite))=='modalite_va_unique' && $modalite->type_reponse=='texte') 
			continue; //ignorer les modalitÃ©s Ã  rÃ©ponse texte 
...
/***
 * Cette fonction affiche la liste des critÃ¨res (choix)
 */
...
function progressbar($val){
	...
	<div class='smiley_1'></div>";
		...
		//Affichage par dÃ©faut, si le paramÃ©trage n'est pas fait
...
<div id="top_r"></div>
	<div id="m_contenu">		
		<form name="theForm" action="" method="post">
			<input type="hidden" name="action"  value="A0">
			<table border="0">
	   			<tr>
	   			...
	   			<tr>
	     			<td>Type de suivi </td>
	     			<td>
						<select name="type_suivi" onChange="this.form.elements['action'].value= 'A0'; this.form.submit();">
							<option value="cfa" <?php if($type_suivi_select == 'cfa') echo'selected'; ?> > 
								<?php echo $config_lea->appelation_suivi_cfa; ?> </option>
							<option value="entr" <?php if($type_suivi_select == 'entr') echo'selected'; ?> > 
								<?php echo $config_lea->appelation_suivi_entr; ?> </option>
						</select>
					</td>
				</tr>
          			<?php
